...
def train_single_instance(dataset_path, epochs_per_iteration, model):
    """Train a single YOLO instance and return the best model path and its performance."""
    if model is not None:
        model = YOLO(model)             # load the teacher model
    else:
        model = YOLO('yolov8n.pt')    # load a pretrained model

    # Train the model
    results = model.train(data=f'{dataset_path}/data.yaml', epochs=epochs_per_iteration, imgsz=img_size, plots=True, patience = 5 )
    save_dir = results.save_dir  # Get the save directory
    best_model_path = os.path.join(save_dir, 'weights', 'best.pt')  # Construct the path to the best model

    # Retrieve and save performance metrics
    metrics = results.maps.mean()
...
def train_multiple_instances(datasets, epochs_per_iteration, trained_models_paths, img_size):
    ...
    for dataset, model in zip(datasets, trained_models_paths):
        best_model_path, metrics = train_single_instance(dataset, epochs_per_iteration, model, img_size)
        best_model_paths.append(best_model_path)
        model_performance.append(metrics)
...
def Pseudo_Labeling(best_model_paths, unlabeled_dataset, iteration):
    ...
    for i, model_path in enumerate(best_model_paths):
        ...
        labels_source = Path(f'{HOME}/runs/detect/predict/labels')
